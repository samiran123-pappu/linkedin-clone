...
const ProfileHeader = ({ userData, authUser, onSave, isOwnProfile }) => {
    ...
    const { data: connectionStatus, refetch: refetchConnectionStatus } = useQuery({
        ...
        queryFn: () => axiosInstance.get(`/connections/status/${profileUserId}`),
        enabled: !isOwnProfile && !!profileUserId,
    });

    const isConnected = authUser && userData?.connections?.some((connection) => connection === authUser._id);
        ...
        switch (getConnectionStatus) {
                ...
                return (
                    <div className='flex gap-2 justify-center'>
                        ...
                        <button
                            ...
                            onClick={() => userData?._id && removeConnection(userData._id)}
            ...
            case "received":
                return (
                    <div className='flex gap-2 justify-center'>
                        <button
                            onClick={() => connectionStatus?.data?.requestId && acceptRequest(connectionStatus.data.requestId)}
                            disabled={!connectionStatus?.data?.requestId}
                            className={`${baseClass} bg-green-500 hover:bg-green-600`}
                        >
                            Accept
                        </button>
                        <button
                            onClick={() => connectionStatus?.data?.requestId && rejectRequest(connectionStatus.data.requestId)}
                            disabled={!connectionStatus?.data?.requestId}
            ...
            default:
                return (
                    <button
                        onClick={() => userData?._id && sendConnectionRequest(userData._id)}
                        disabled={!userData?._id}
                        className='bg-primary hover:bg-primary-dark disabled:bg-gray-400 disabled:cursor-not-allowed text-white py-2 px-4 rounded-full transition duration-300 flex items-center justify-center'
                    >
